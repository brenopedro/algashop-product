...
public class StorageProviderAwsS3Impl implements StorageProvider {
    ...
    public void deleteFile(String remoteFileName) {
        if (!this.fileExists(remoteFileName))
            throw new StorageProviderException(String.format("Remote file %s already exists", remoteFileName));

        try {
            s3Template.deleteObject(properties.getBucketName(), remoteFileName);
        } catch (S3Exception e) {
            throw new StorageProviderException(String.format("Unknown error when tried to remove " +
                    "the file %s", remoteFileName), e);
        }
    }
